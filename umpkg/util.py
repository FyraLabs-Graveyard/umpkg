...
class Command:
    ...
    def push(self, tag, branch=None):
        ...
        if cfg["git_repo"] != "":
            ...
            gitLink = (
                subprocess.run(
                    ["git", "remote", "get-url", "origin"], stdout=subprocess.PIPE
                )
                .stdout.decode("utf-8")
                .strip()
            )
        ...
        if not branch:
            ...
            branch = tag
        gitCommit = (
            subprocess.run(["git", "rev-parse", branch], stdout=subprocess.PIPE)
            .stdout.decode("utf-8")
            .strip()
        )
        if gitCommit == branch:
        ...
        if (
            not subprocess.run(
                ["git", "cherry", "-v", "origin/" + branch], stdout=subprocess.PIPE
            )
            .stdout.decode("utf-8")
            .strip()
            == ""
